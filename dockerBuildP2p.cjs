...
const dockerfilePath = "./Dockerfile.p2pNode";
...
rl.question(
  ...
  (inputVersion) => {
    ...
    const imageName = `${process.env.DOCKER_REGISTRY}/${process.env.DOCKER_IMAGE_NAME_P2P}:${VERSION}`;
    ...
    rl.question(`Use cache, y or n (default: y): `, async (useCache) => {
      ...
      try {
        ...
        console.log(
          `\nDocker image pushed successfully to ${process.env.DOCKER_REGISTRY} as ${process.env.DOCKER_IMAGE_NAME_P2P}:${VERSION}\n`
        );
      } catch (error) {
        console.error("\nError occurred:\n", error);
        return;
